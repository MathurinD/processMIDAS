...
#' @example mergeMIDAS("blunt1_MIDAS.csv", "blunt2_MIDAS.csv")
#' @seealso extractMIDAS
mergeMIDAS <- function(..., save_file=FALSE) {
    mfiles = unlist(list(...))
    all_ids = c()
    all_measurements = c()
    all_treatments = c()
    ...
    for (ff in mfiles) {
        ...
        midas_list[[ff]] = output
    }
    save_midas = midas_list[[1]]
    print(nrow(midas_list[[1]]))
